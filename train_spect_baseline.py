...
class SPECTEncoder(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(28224, 128)
...
def parse_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=3e-05) # 1e-03
...
def main():
    ...
    for run in range(args.runs):
        ...
        for fold in range(args.folds):
            ...
            model = SPECTEncoder().to(device)
            model.load_state_dict(torch.load(vis_model_path))

            _, _, _, eval_acc, eval_auc = evaluate(args, model, device, test_loader, args.meth, args.spect, args.num_datasets, criterion)
